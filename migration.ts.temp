...
async function processBatch(
    ...
    for (const user of users) {
        ...
        // Map simplified UserInsertData keys to actual DB column names for the SQL query
        ...
        if (userData.isAnonymous !== undefined) mappedUserData.is_anonymous = userData.isAnonymous;
        if (userData.phoneNumber) mappedUserData.phone_number = userData.phoneNumber;
        if (userData.phoneNumberVerified !== undefined)
            mappedUserData.phone_number_verified = userData.phoneNumberVerified;
        if (userData.role) mappedUserData.role = userData.role;
        if (userData.banned !== undefined) mappedUserData.banned = userData.banned;
        if (userData.banExpires) mappedUserData.ban_expires = userData.banExpires;
        if (userData.banReason) mappedUserData.ban_reason = userData.banReason;
        if (userData.userMetadata) mappedUserData.user_metadata = userData.userMetadata;
        if (userData.appMetadata) mappedUserData.app_metadata = userData.appMetadata;
        if (userData.invitedAt) mappedUserData.invited_at = userData.invitedAt;
        if (userData.lastSignInAt) mappedUserData.last_sign_in_at = userData.lastSignInAt;

        validUsersData.push({ user, userData: mappedUserData as any }); // Use 'as any' to bypass the type check for now since we transformed keys
    }
