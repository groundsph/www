...
import {
    CAFE_CRAWL_REPORT_REASONS,
    CafeCrawlReportReason,
} from "@/utils/types/cafe-crawl-reports"
import { reportCafeCrawl } from "@/app/api/actions/cafe-crawls"
import { useAuth } from "@/components/layout/AuthProvider"
import { cn } from "@/utils/cn"

interface CrawlReportButtonProps {
    crawlId: string
    className?: string
}

export default function CrawlReportButton({ crawlId, className }: CrawlReportButtonProps) {
    const { user } = useAuth()
    const [isOpen, setIsOpen] = useState(false)
    const [reason, setReason] = useState<CafeCrawlReportReason | "">("")
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const result = await reportCafeCrawl({
                crawlId,
                reason: reason as CafeCrawlReportReason,
    ...
    return (
        <>
            <button
                ...
                className={cn(
                    "flex items-center gap-2 px-4 py-2 text-sm font-medium text-text/70 hover:text-red-500 hover:bg-red-500/10 border border-secondary/30 rounded-full transition-colors",
                    className
                )}
                                    ...
                                    <div className='space-y-2'>
                                        ...
                                        <select
                                            ...
                                            onChange={(e) =>
                                                setReason(
                                                    e.target
                                                        .value as CafeCrawlReportReason
                                                )
                                            }
                                            required
                                        ...
                                        >
                                            ...
                                            {CAFE_CRAWL_REPORT_REASONS.map((r) => (
                                                <option
                                                    key={r.value}
                                                    value={r.value}
                                                >
