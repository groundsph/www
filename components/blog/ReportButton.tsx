...
export default function ReportButton({ postId, className }: ReportButtonProps) {
    ...
    const [reason, setReason] = useState<BlogReportReason | "">("")
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const result = await reportBlogPost({
                blog_post_id: postId,
                reason: reason as BlogReportReason,
    ...
    return (
        <>
            <button
                ...
                className={cn(
                    "flex items-center gap-2 px-3 py-1.5 text-sm font-medium text-text/60 hover:text-red-500 hover:bg-red-500/10 rounded-lg transition-colors",
                    className
                )}
