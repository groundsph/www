...
\restrict cB5Vv5fNvibX8XCWsQTYX1OtzddHYUnQXD9TkdlHrx8ns1Bi9w4VA1JR6OQuKMT
...
CREATE POLICY "Active features are viewable by everyone" ON public.featured_schedules FOR SELECT USING ((is_active = true));
...
CREATE POLICY "Admins and creators can delete events" ON public.events FOR DELETE USING (((created_by = auth.uid()) OR (EXISTS ( SELECT 1
...
CREATE POLICY "Admins and creators can update events" ON public.events FOR UPDATE USING (((created_by = auth.uid()) OR (EXISTS ( SELECT 1
   FROM public.profiles
  WHERE ((profiles.id = auth.uid()) AND (profiles.role = ANY (ARRAY['admin'::public.user_role, 'moderator'::public.user_role])))))));
...
CREATE POLICY "Admins and mods can read all events" ON public.events FOR SELECT USING ((EXISTS ( SELECT 1
   FROM public.profiles
  WHERE ((profiles.id = auth.uid()) AND (profiles.role = ANY (ARRAY['admin'::public.user_role, 'moderator'::public.user_role]))))));
...
CREATE POLICY "Admins can delete cafes" ON public.cafes FOR DELETE TO authenticated USING ((EXISTS ( SELECT 1
   FROM public.profiles
  WHERE ((profiles.id = auth.uid()) AND (profiles.role = ANY (ARRAY['admin'::public.user_role, 'moderator'::public.user_role]))))));
...
CREATE POLICY "Admins can insert events" ON public.events FOR INSERT WITH CHECK ((EXISTS ( SELECT 1
   FROM public.profiles
  WHERE ((profiles.id = auth.uid()) AND (profiles.role = ANY (ARRAY['admin'::public.user_role, 'moderator'::public.user_role]))))));
...
CREATE POLICY "Admins can manage all suggestions" ON public.cafe_edit_suggestions USING ((EXISTS ( SELECT 1
   FROM public.profiles
  WHERE ((profiles.id = auth.uid()) AND (profiles.role = ANY (ARRAY['admin'::public.user_role, 'moderator'::public.user_role]))))));
...
CREATE POLICY "Admins can see all reports" ON public.blog_reports USING ((EXISTS ( SELECT 1
   FROM public.profiles
  WHERE ((profiles.id = auth.uid()) AND (profiles.role = ANY (ARRAY['admin'::public.user_role, 'moderator'::public.user_role]))))));
...
CREATE POLICY "Admins can update cafes" ON public.cafes FOR UPDATE TO authenticated USING ((EXISTS ( SELECT 1
   FROM public.profiles
  WHERE ((profiles.id = auth.uid()) AND (profiles.role = ANY (ARRAY['admin'::public.user_role, 'moderator'::public.user_role])))))) WITH CHECK (true);
...
CREATE POLICY "Admins can update claims" ON public.cafe_claims FOR UPDATE USING ((EXISTS ( SELECT 1
   FROM public.profiles
  WHERE ((profiles.id = auth.uid()) AND (profiles.role = ANY (ARRAY['admin'::public.user_role, 'moderator'::public.user_role]))))));
...
CREATE POLICY "Admins can update verification requests" ON public.owner_verification_requests FOR UPDATE USING ((EXISTS ( SELECT 1
   FROM public.profiles
  WHERE ((profiles.id = auth.uid()) AND (profiles.role = ANY (ARRAY['admin'::public.user_role, 'moderator'::public.user_role]))))));
...
CREATE POLICY "Admins can view all claims" ON public.cafe_claims FOR SELECT USING ((EXISTS ( SELECT 1
   FROM public.profiles
  WHERE ((profiles.id = auth.uid()) AND (profiles.role = ANY (ARRAY['admin'::public.user_role, 'moderator'::public.user_role]))))));
...
CREATE POLICY "Admins full access" ON public.blog_posts USING ((auth.uid() IN ( SELECT profiles.id
   FROM public.profiles
...
CREATE POLICY "Admins have full access" ON public.cafe_menu_items TO authenticated USING ((EXISTS ( SELECT 1
   FROM public.profiles
  WHERE ((profiles.id = auth.uid()) AND (profiles.role = ANY (ARRAY['admin'::public.user_role, 'moderator'::public.user_role])))))) WITH CHECK ((EXISTS ( SELECT 1
   FROM public.profiles
  WHERE ((profiles.id = auth.uid()) AND (profiles.role = ANY (ARRAY['admin'::public.user_role, 'moderator'::public.user_role]))))));
...
CREATE POLICY "Allow all inserts to rating stats" ON public.cafe_rating_stats TO authenticated USING (true) WITH CHECK (true);
...
CREATE POLICY "Anyone can track views" ON public.cafe_page_views FOR INSERT WITH CHECK (true);
...
CREATE POLICY "Anyone can view menu items for published cafes" ON public.cafe_menu_items FOR SELECT USING (((EXISTS ( SELECT 1
   FROM public.cafes
  WHERE ((cafes.id = cafe_menu_items.cafe_id) AND (cafes.is_published = true)))) OR (EXISTS ( SELECT 1
   FROM public.cafes
  WHERE ((cafes.id = cafe_menu_items.cafe_id) AND (cafes.owner_ids @> ARRAY[auth.uid()]))))));
...
CREATE POLICY "Anyone can view review responses" ON public.owner_review_responses FOR SELECT USING (true);
...
CREATE POLICY "Authenticated users can insert suggestions" ON public.cafe_edit_suggestions FOR INSERT WITH CHECK ((auth.uid() = user_id));
...
CREATE POLICY "Authenticated users can post reviews" ON public.reviews FOR INSERT WITH CHECK ((auth.role() = 'authenticated'::text));
...
CREATE POLICY "Authenticated users can submit cafes" ON public.cafes FOR INSERT TO authenticated WITH CHECK ((is_published IS NOT TRUE));
...
CREATE POLICY "Authors manage own drafts" ON public.blog_posts USING (((author_id = auth.uid()) AND (status = 'draft'::public.blog_status)));
...
CREATE POLICY "Cafe owners can insert events" ON public.events FOR INSERT WITH CHECK (((cafe_id IS NOT NULL) AND (EXISTS ( SELECT 1
   FROM public.cafes
  WHERE ((cafes.id = events.cafe_id) AND (auth.uid() = ANY (cafes.owner_ids)))))));
...
CREATE POLICY "Contribution logs are viewable by everyone" ON public.contribution_logs FOR SELECT USING (true);
...
CREATE POLICY "Contributors can update own unpublished cafes" ON public.cafes FOR UPDATE TO authenticated USING (((contributor_id = auth.uid()) AND (is_published = false))) WITH CHECK ((is_published = false));
...
CREATE POLICY "Creators can read own events" ON public.events FOR SELECT USING ((created_by = auth.uid()));
...
CREATE POLICY "Interactions are public" ON public.review_interactions FOR SELECT USING (true);
...
CREATE POLICY "Owners can create responses for their cafe reviews" ON public.owner_review_responses FOR INSERT WITH CHECK ((EXISTS ( SELECT 1
   FROM (public.reviews r
     JOIN public.cafes c ON ((r.cafe_id = c.id)))
  WHERE ((r.id = owner_review_responses.review_id) AND (c.owner_ids @> ARRAY[auth.uid()])))));
...
CREATE POLICY "Owners can delete their own responses" ON public.owner_review_responses FOR DELETE USING ((owner_id = auth.uid()));
...
CREATE POLICY "Owners can manage stories for their cafes" ON public.cafe_stories USING ((EXISTS ( SELECT 1
   FROM public.cafes
  WHERE ((cafes.id = cafe_stories.cafe_id) AND (auth.uid() = ANY (cafes.owner_ids))))));
...
CREATE POLICY "Owners can manage their cafe menu items" ON public.cafe_menu_items USING ((EXISTS ( SELECT 1
   FROM public.cafes
  WHERE ((cafes.id = cafe_menu_items.cafe_id) AND (cafes.owner_ids @> ARRAY[auth.uid()])))));
...
CREATE POLICY "Owners can read their cafe views" ON public.cafe_page_views FOR SELECT USING ((cafe_id IN ( SELECT cafes.id
   FROM public.cafes
  WHERE (auth.uid() = ANY (cafes.owner_ids)))));
...
CREATE POLICY "Owners can update their own responses" ON public.owner_review_responses FOR UPDATE USING ((owner_id = auth.uid()));
...
CREATE POLICY "Owners can view their cafe subscription" ON public.cafe_subscriptions FOR SELECT USING (((cafe_id IN ( SELECT cafes.id
   FROM public.cafes
  WHERE (cafes.owner_ids @> ARRAY[auth.uid()]))) OR (EXISTS ( SELECT 1
   FROM public.profiles
  WHERE ((profiles.id = auth.uid()) AND (profiles.role = ANY (ARRAY['admin'::public.user_role, 'moderator'::public.user_role])))))));
...
CREATE POLICY "Owners manage cafe posts" ON public.blog_posts USING (((cafe_id IS NOT NULL) AND (auth.uid() IN ( SELECT unnest(cafes.owner_ids) AS unnest
   FROM public.cafes
...
CREATE POLICY "Public can read published events" ON public.events FOR SELECT USING ((status = 'published'::public.event_status));
...
CREATE POLICY "Public can read published posts" ON public.blog_posts FOR SELECT USING ((status = 'published'::public.blog_status));
...
CREATE POLICY "Published cafes are viewable by everyone" ON public.cafes FOR SELECT USING ((is_published = true));
...
CREATE POLICY "Reviews are viewable by everyone" ON public.reviews FOR SELECT USING ((status = 'published'::public.review_status));
...
CREATE POLICY "Scouters can manage stories for their cafes" ON public.cafe_stories USING ((EXISTS ( SELECT 1
   FROM public.cafes
  WHERE ((cafes.id = cafe_stories.cafe_id) AND (cafes.contributor_id = auth.uid())))));
...
CREATE POLICY "Service role can manage avatar deletion queue" ON public.avatar_deletion_queue USING (true) WITH CHECK (true);
...
CREATE POLICY "Service role can manage subscriptions" ON public.cafe_subscriptions USING (((auth.jwt() ->> 'role'::text) = 'service_role'::text));
...
CREATE POLICY "Stats are viewable by everyone" ON public.cafe_rating_stats FOR SELECT USING (true);
...
CREATE POLICY "Stories are viewable by everyone" ON public.cafe_stories FOR SELECT USING ((EXISTS ( SELECT 1
   FROM public.cafes
...
CREATE POLICY "Users can create interactions" ON public.review_interactions FOR INSERT WITH CHECK ((auth.role() = 'authenticated'::text));
...
CREATE POLICY "Users can create verification requests" ON public.owner_verification_requests FOR INSERT WITH CHECK ((user_id = auth.uid()));
...
CREATE POLICY "Users can delete their own interactions" ON public.review_interactions FOR DELETE USING ((auth.uid() = user_id));
...
CREATE POLICY "Users can insert claims" ON public.cafe_claims FOR INSERT WITH CHECK ((auth.uid() = user_id));
...
CREATE POLICY "Users can modify their own reviews" ON public.reviews USING ((auth.uid() = user_id));
...
CREATE POLICY "Users can see own reports" ON public.blog_reports FOR SELECT USING ((auth.uid() = reporter_id));
...
CREATE POLICY "Users can view own claims" ON public.cafe_claims FOR SELECT USING ((auth.uid() = user_id));
...
CREATE POLICY "Users can view own submissions" ON public.cafes FOR SELECT TO authenticated USING ((contributor_id = auth.uid()));
...
CREATE POLICY "Users can view own suggestions" ON public.cafe_edit_suggestions FOR SELECT USING ((auth.uid() = user_id));
...
CREATE POLICY "Users can view their verification requests" ON public.owner_verification_requests FOR SELECT USING (((user_id = auth.uid()) OR (EXISTS ( SELECT 1
   FROM public.profiles
  WHERE ((profiles.id = auth.uid()) AND (profiles.role = ANY (ARRAY['admin'::public.user_role, 'moderator'::public.user_role])))))));
...
ALTER TABLE public.avatar_deletion_queue ENABLE ROW LEVEL SECURITY;
...
ALTER TABLE public.cafe_page_views ENABLE ROW LEVEL SECURITY;

--
-- Name: cafe_rating_stats; Type: ROW SECURITY; Schema: public; Owner: -
--

ALTER TABLE public.cafe_rating_stats ENABLE ROW LEVEL SECURITY;

--
-- Name: cafe_stories; Type: ROW SECURITY; Schema: public; Owner: -
--

ALTER TABLE public.cafe_stories ENABLE ROW LEVEL SECURITY;
...
ALTER TABLE public.featured_schedules ENABLE ROW LEVEL SECURITY;

--
-- Name: owner_review_responses; Type: ROW SECURITY; Schema: public; Owner: -
--

ALTER TABLE public.owner_review_responses ENABLE ROW LEVEL SECURITY;
